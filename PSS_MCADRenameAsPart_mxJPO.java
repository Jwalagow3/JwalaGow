...
public class PSS_MCADRenameAsPart_mxJPO extends IEFCommonUIActions_mxJPO {
    ...
    protected static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger("PSS_MCADRenameAsPart");
    ...
    public void executeCustom(Context _context, String strNewName, String strOldName) throws MCADException {
        logger.debug("::::::::::::::: ENTER :: executeCustom :::::::::::::::");
        ...
        logger.debug("::::::::::::::: EXIT :: executeCustom :::::::::::::::");
            ...
            while (allRels.hasMoreElements()) {
                ...
                if (bRenameOnlyInstance) {
                    ...
                    if (MCADUtil.areStringsEqual(boName, oldBusName, isSystemCaseSensitive))
                        _util.setRelationshipAttributeValue(_context, rel, attribName, changedName);
                    /* START :: TIGTK-18444 :: ALM6581 */
                    // else if (boName.startsWith(oldBusName + "."))
                    else if (boName.startsWith(oldBusName + ".") || boName.startsWith(oldBusName + "_")) {
                        /* END :: TIGTK-18444 :: ALM6581 */
                        _util.setRelationshipAttributeValue(_context, rel, attribName, newBusName);
                    }
                    validateBusObjectName(changedName, cadType);
                    if (!alreadyRenamedObjList.contains(ddBO.getObjectId())) {
    ...
    public MapList getCADStructureForRename(Context context, String[] args) throws Exception {
        ...
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
            System.out.println("--852-paramMap---"+paramMap);
            String strCADObjId = (String) paramMap.get("objectId");
            // START :: TIGTK-18444 :: ALM6581
            if (UIUtil.isNullOrEmpty(strCADObjId)) {
                return mpReturnList;
            }
            // END :: TIGTK-18444 :: ALM6581
            ...
            //DomainObject doParentObj = DomainObject.newInstance(context,strParentId);
           // Strig strParentType
            String strRelPattern = REALTIONSHIP_CAD_SUBCOMPONENT+","+TigerConstants.RELATIONSHIP_ASSOCIATEDDRAWING;
           //ACN US 5717 END
            boolean bisTo = false;
            ...
            System.out.println("-----881--parentId--"+strParentId);
            System.out.println("-----881--strRelPattern--"+strRelPattern);
            System.out.println("-----881--strRelPattern--"+bisTo);
            System.out.println("-----881--strRelPattern--"+bisFrom);
            ...
            System.out.println("----889---mpListExpand---"+mpListExpand);
            iter.close();
            ...
            for (int i = 0; i < mpListExpand.size(); i++) {
                ...
                //ACN US 5717 START
                if(!strParentId.equals(strObjIdExpand))
        ...
        try {
            ...
            if (!strOldName.equals(strNewName)) {
                ...
                if (UIUtil.isNotNullAndNotEmpty(strPDFArchiveOID)) {
                    ...
                    // START :: TIGTK-18444 :: ALM6581
                    if (UIUtil.isNotNullAndNotEmpty(strThumbnailOID)) {
                        // END :: TIGTK-18444 :: ALM6581
